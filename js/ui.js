...
function renderLeave() {
    ...
    let filtered = leaves; if(!canManageLeave) filtered = leaves.filter(l => l.employeeId === user.uid);
...
function renderPayroll() {
    ...
    let filtered = payroll; if(!canManagePayroll) filtered = payroll.filter(p => p.employeeId === user.uid);
...
function renderInventory() {
    ...
    if(filtered.length === 0) { tbody.innerHTML = `<tr><td colspan="8" class="text-center text-muted">No items.</td></tr>`; return; }
    const canEdit = window.canManage('inventory');
    tbody.innerHTML = filtered.map(i => `<tr><td><code>${escapeHtml(i.productCode||'—')}</code></td><td><code>${escapeHtml(i.barcode||'—')}</code></td><td><strong>${escapeHtml(i.name)}</strong></td><td>${escapeHtml(i.category||'—')}</td><td class="${(i.qty||0)<=5?'text-danger':''}">${i.qty||0}</td><td>LKR ${formatCurrency(i.price||0)}</td><td><span class="badge ${i.status==='active'?'badge-success':'badge-danger'}">${i.status||'active'}</span></td><td class="text-center">${canEdit ? `<button class="btn btn-sm btn-outline" onclick="editItem('${i.id}')"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="deleteItem('${i.id}')"><i class="fas fa-trash"></i></button>` : '—'}</td></tr>`).join('');
}
...
function renderProducts() {
    ...
    // Update attributes panel with first item data
    const firstItem = items[0];
    if (firstItem) {
        document.getElementById('productCode').value = firstItem.productCode || ('PRD-' + String(items.length).padStart(4, '0'));
        document.getElementById('productUnit').value = firstItem.unit || 'Pcs';
        document.getElementById('productCostPrice').value = firstItem.costPrice || 0;
        document.getElementById('productTaxRate').value = firstItem.taxRate || 0;
        document.getElementById('productReorderLevel').value = firstItem.reorderLevel || 0;
        document.getElementById('productStockAlert').value = firstItem.stockAlert || 'enabled';
    } else {
        document.getElementById('productCode').value = 'PRD-' + String(items.length + 1).padStart(4, '0');
        ...
        document.getElementById('productStockAlert').value = 'enabled';
    }
}
...
function renderDeliveries() {
    ...
    const data = getAppData();
    // Populate basic dropdowns (calls app.js function if available)
    if (typeof window.populateDeliveryDropdowns === 'function') {
        window.populateDeliveryDropdowns();
    }

    // Populate delivery cart item dropdown
    const cartItemSelect = document.getElementById('delCartItemSelect');
    if (cartItemSelect) {
        const val = cartItemSelect.value;
        cartItemSelect.innerHTML = '<option value="">Select Item</option>' + 
...
function renderReports() {
    ...
    switch(type) {
        case 'stock': {
            ...
            else html = `<table><thead><tr><th>Code</th><th>Item</th><th>Category</th><th>Qty</th><th>Price</th><th>Value</th></tr></thead><tbody>${items.map(i => `<tr><td>${escapeHtml(i.productCode||'—')}</td><td>${escapeHtml(i.name)}</td><td>${escapeHtml(i.category||'—')}</td><td>${i.qty||0}</td><td>LKR ${formatCurrency(i.price||0)}</td><td>LKR ${formatCurrency((i.qty||0)*(i.price||0))}</td></tr>`).join('')}</tbody></table>`;
            break;
        }
        case 'sales': {
        ...
        case 'finance': {
            ...
            else { const inc = fin.filter(f => f.type==='income').reduce((s,f) => s+(f.amount||0),0); const exp = fin.filter(f => f.type==='expense').reduce((s,f) => s+(f.amount||0),0); html = `<table><thead><tr><th>Date</th><th>Type</th><th>Category</th><th>Method</th><th>Description</th><th>Amount</th></tr></thead><tbody>${fin.map(f => `<tr><td>${formatDate(f.date)}</td><td><span class="badge ${f.type==='income'?'badge-success':'badge-danger'}">${f.type}</span></td><td>${escapeHtml(f.category||'—')}</td><td>${escapeHtml(f.paymentMethod||'cash')}</td><td>${escapeHtml(f.desc||'—')}</td><td>${f.type==='income'?'+':'-'} LKR ${formatCurrency(f.amount||0)}</td></tr>`).join('')}</tbody><tfoot><tr><td colspan="5"><strong>Income: ${formatCurrency(inc)} | Expense: ${formatCurrency(exp)} | Balance: ${formatCurrency(inc-exp)}</strong></td></tr></tfoot></table>`; }
            break;
        }
...
window.editEmployee = function(id) {
    const data=getAppData(); const emp=data.employees.find(e=>e.id===id); if(!emp) return;
    const setVal=(id,val)=>{const el=document.getElementById(id); if(el) el.value=val;};
    setVal('empEditId', emp.id); setVal('empName', emp.name||''); setVal('empNIC', emp.nic||''); setVal('empDept', emp.department||'Admin');
    setVal('empDesignation', emp.designation||''); setVal('empContact', emp.contact||''); setVal('empEmergency', emp.emergency||'');
    setVal('empAddress', emp.address||''); setVal('empJoined', emp.joinedDate||''); setVal('empSalary', emp.salary||'');
    setVal('empEpf', emp.epf||''); setVal('empUsername', emp.email||''); setVal('empPassword', ''); setVal('empStatus', emp.status||'active');
    document.getElementById('employeeModalTitle').textContent='✏️ Edit Employee'; document.getElementById('employeeModal').classList.add('open');
};
window.deleteEmployee = async function(id) {
    if(!confirm('Delete?')) return; const data=getAppData(); data.employees=data.employees.filter(e=>e.id!==id); if(data.leaveBalances) delete data.leaveBalances[id]; setAppData(data); await saveAllData(); renderEmployees(); showToast('🗑️ Removed.');
};
window.editItem = function(id) {
    const data=getAppData(); const item=data.items.find(i=>i.id===id); if(!item) return;
    const setVal=(id,val)=>{const el=document.getElementById(id); if(el) el.value=val;};
    setVal('itemEditId', item.id); setVal('itemBarcode', item.barcode||''); setVal('itemName', item.name||'');
    setVal('itemQty', item.qty||0); setVal('itemPrice', item.price||0); setVal('itemCategory', item.category||'');
    setVal('itemBrand', item.brand||''); setVal('itemDesc', item.desc||''); setVal('itemExpiry', item.expiry||'');
    setVal('itemBatch', item.batch||''); setVal('itemStatus', item.status||'active');
    setVal('itemProductCode', item.productCode||''); setVal('itemUnit', item.unit||'Pcs');
    setVal('itemCostPrice', item.costPrice||0); setVal('itemReorderLevel', item.reorderLevel||0);
    setVal('itemTaxRate', item.taxRate||0); setVal('itemStockAlert', item.stockAlert||'enabled');
    document.getElementById('itemModalTitle').textContent='✏️ Edit Item'; document.getElementById('itemModal').classList.add('open'); populateItemDropdowns();
};
window.deleteItem = async function(id) {
    if(!confirm('Delete?')) return; const data=getAppData(); data.items=data.items.filter(i=>i.id!==id); setAppData(data); await saveAllData(); renderInventory(); showToast('🗑️ Removed.');
};
window.editCustomer = function(id) {
    const data=getAppData(); const c=data.customers.find(c=>c.id===id); if(!c) return;
    const setVal=(id,val)=>{const el=document.getElementById(id); if(el) el.value=val;};
    setVal('custEditId', c.id); setVal('custName', c.name||''); setVal('custContact', c.contact||'');
    setVal('custCategory', c.category||'Retail'); setVal('custAddress', c.address||'');
    setVal('custCreditLimit', c.creditLimit||0); setVal('custBalance', c.balance||0);
    document.getElementById('customerModalTitle').textContent='✏️ Edit Customer'; document.getElementById('customerModal').classList.add('open');
};
window.deleteCustomer = async function(id) {
    if(!confirm('Delete?')) return; const data=getAppData(); data.customers=data.customers.filter(c=>c.id!==id); setAppData(data); await saveAllData(); renderCustomers(); showToast('🗑️ Removed.');
};
window.editVehicle = function(id) {
    const data=getAppData(); const v=data.vehicles.find(v=>v.id===id); if(!v) return;
    const setVal=(id,val)=>{const el=document.getElementById(id); if(el) el.value=val;};
    setVal('vehicleNo', v.vehicleNo||''); setVal('vehicleDriver', v.driver||''); setVal('vehicleFuel', v.fuel||'');
    setVal('vehicleInsurance', v.insurance||''); setVal('vehicleService', v.service||'');
    const btn=document.getElementById('addVehicleBtn'); if(btn) { btn.dataset.editId=v.id; btn.textContent='💾 Update Vehicle'; }
    showToast('✏️ Editing.');
};
window.deleteVehicle = async function(id) {
    if(!confirm('Delete?')) return; const data=getAppData(); data.vehicles=data.vehicles.filter(v=>v.id!==id); setAppData(data); await saveAllData(); renderFleet(); showToast('🗑️ Removed.');
};
window.deleteSupplier = async function(id) {
    if(!confirm('Delete supplier?')) return; const data=getAppData(); data.suppliers=data.suppliers.filter(s=>s.id!==id); setAppData(data); await saveAllData(); renderPurchasing(); showToast('🗑️ Removed.');
};
window.removeCategory = async function(cat) {
    if(!confirm(`Remove "${cat}"?`)) return; const data=getAppData(); data.categories=data.categories.filter(c=>c!==cat); data.items.forEach(i=>{if(i.category===cat) i.category='';}); setAppData(data); await saveAllData(); renderProducts(); renderInventory(); showToast('🗑️ Removed.');
};
window.removeBrand = async function(brand) {
    if(!confirm(`Remove "${brand}"?`)) return; const data=getAppData(); data.brands=data.brands.filter(b=>b!==brand); data.items.forEach(i=>{if(i.brand===brand) i.brand='';}); setAppData(data); await saveAllData(); renderProducts(); showToast('🗑️ Removed.');
};
```
